...
resource "aws_instance" "sonarqube-server" {
  ...
  user_data = templatefile("${path.module}/sonarqube-userdata.sh.tpl", {
    sonarqube_role_id   = var.sonarqube_role_id,
    sonarqube_secret_id = var.sonarqube_secret_id
    SONAR_VERSION       = var.sonarqube_version 
    SONAR_ZIP           = var.sonar_zip  
  })
